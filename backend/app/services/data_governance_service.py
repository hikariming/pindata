...
class DataGovernanceService:
    ...
    def get_projects(
        ...
        # 基础查询 - 获取用户有权限访问的项目
        query = self.db.query(DataGovernanceProject).join(
            ProjectTeamMember,
        ...
        # 组织过滤
        if organization_id:
            query = query.filter(DataGovernanceProject.organization_id == organization_id)
        
        # 状态过滤
        if status_filter and status_filter != "all":
            query = query.filter(DataGovernanceProject.status == ProjectStatus[status_filter.upper()])
        ...
        for project in projects:
            # 获取团队成员信息
            team_members = self.db.query(ProjectTeamMember).join(User).filter(
            ...
            project_data = {
                ...
                "metrics": {
                    ...
                    "lastProcessedAt": project.last_processed_at.isoformat() if project.last_processed_at else None,
    ...
    def get_project_by_id(self, project_id: int, user_id: int) -> Optional[Dict[str, Any]]:
        """根据ID获取项目详情"""
        
        # 检查用户是否有权限访问此项目
        project = self.db.query(DataGovernanceProject).join(
            ProjectTeamMember,
            DataGovernanceProject.id == ProjectTeamMember.project_id
        ).filter(
            DataGovernanceProject.id == project_id,
            ProjectTeamMember.user_id == user_id,
            ProjectTeamMember.status == MemberStatus.ACTIVE
        ).first()
    ...
    def create_project(
        ...
        # 返回创建的项目信息
        return self.get_project_by_id(project.id, user_id)
    ...
    def update_project(
        ...
        **kwargs
        ...
        team_member = self.db.query(ProjectTeamMember).filter(
        ...
        # 获取项目
        project = self.db.query(DataGovernanceProject).filter(
            DataGovernanceProject.id == project_id
        ).first()
        ...
        # 更新字段
        updatable_fields = ['name', 'description', 'status', 'config']
    ...
    def get_user_project_stats(self, user_id: int, organization_id: Optional[int] = None) -> Dict[str, Any]:
        """获取用户的项目统计信息"""
        ...
        for project in projects:
            members = self.db.query(ProjectTeamMember).filter(
                ProjectTeamMember.project_id == project.id,
                ProjectTeamMember.status == MemberStatus.ACTIVE
            ).all()
